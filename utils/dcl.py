...
def get_dcl_channel(name, interval):
    return get_code_len(interval, mode=return_mode(name))

def get_code_len(class_range=180, mode=0):
    if mode in [0, 1, 3]:
...
def binary_label_encode(angle_label, angle_range=180, omega=1.):
    """
    ...
    angle_label = angle_label.cpu().numpy()
...
def binary_label_decode(binary_label, angle_range=180, omega=1.):
...
def gray_label_encode(angle_label, angle_range=180, omega=1.):
    """
    :param angle_label: [-90,0) or [-180, 0)
    :param angle_range: 90 or 180
    :param omega:
...
def gray_label_decode(gray_label, angle_range=180, omega=1.):
    gray_label = gray_label.cpu().numpy()
    angle_range /= omega
    angle_range = int(angle_range)
...
def dichotomy_label_encode(angle_label, omega, angle_range=180):
    assert (angle_range / omega) % 1 == 0, 'wrong omega'
...
def dichotomy_label_decode(dichotomy_label, angle_range=180, omega=1.):
    assert (angle_range / omega) % 1 == 0, 'wrong omega'
...
def short_dichotomy_label_encode(angle_label, omega, angle_range=180):
    assert (angle_range / omega) % 1 == 0, 'wrong omega'
...
def short_dichotomy_label_decode(dichotomy_label, angle_range=180, omega=1.):
    assert (angle_range / omega) % 1 == 0, 'wrong omega'
...
def dense_label_encode(angle_label, angle_range=180, omega=1., name='BCL'):
...
def dense_label_decode(angle_encode_label, angle_range=180, omega=1.,  name='BCL'):
    mode = return_mode(name)
    if mode == 0:
...
#     binary_label = angle_label_encode([0, 1, 2, 45, 80, 90, 180 - 180 / 256. , 180], 180, 180 / 128., mode=0)
#     print(binary_label)
